...
const App = () => {
  ...
  const [loading, setLoading] = useState(Array(users.length).fill(false));
  const handlerClick = async (idx) => {
    try {
      setLoading((prev) => ({ ...prev, [idx]: true }));
      await new Promise((resolve) => setTimeout(resolve, 2000));
      const updatedUsers = [...users];
      const currentUser = updatedUsers[idx];
      if (currentUser.status === "Stranger") {
        currentUser.status = "Friend";
        currentUser.available = false;
      } else if (currentUser.status === "Friend") {
        currentUser.status = "Stranger";
        currentUser.available = true;
      } else {
        console.warn(`Unexpected status: ${currentUser.status}`);
      }
      setUsers(updatedUsers);
      setLoading((prev) => ({ ...prev, [idx]: false }));
    } catch (error) {
      console.error("Error in handlerClick:", error);
      setLoading((prev) => ({ ...prev, [idx]: false }));
    }
  };
  return (
